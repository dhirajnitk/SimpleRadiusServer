...
import se.nexus.interview.radius.server.response.RadiusPasswordDecoder;
import se.nexus.interview.radius.server.response.RadiusServerException;
import se.nexus.interview.radius.server.response.RadiusUsersAuthenticator;
import se.nexus.interview.radius.server.response.ResponseBuilder;
    ...
    public void process(DatagramPacket packet) throws IOException, NoSuchAlgorithmException, RadiusServerException {
        ...
        while (attributeIndex < length) {
            ...
            if (attributeType == 1) {
                ...
                password = RadiusPasswordDecoder.decode(attributeValue, Arrays.copyOfRange(data, 4, 20));
            }
        ...
        } else if (RadiusUsersAuthenticator.checkCredential(username, password)) {
